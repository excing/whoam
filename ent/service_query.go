...
func (sq *ServiceQuery) QueryMethods() *MethodQuery {
	query := &MethodQuery{config: sq.config}
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withMethods: sq.withMethods.Clone(),
...
func (sq *ServiceQuery) WithMethods(opts ...func(*MethodQuery)) *ServiceQuery {
	query := &MethodQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withMethods = query
	return sq
}
...
//		GroupBy(service.FieldServiceID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (sq *ServiceQuery) sqlAll(ctx context.Context) ([]*Service, error) {
	...
	if query := sq.withMethods; query != nil {
		...
		query.Where(predicate.Method(func(s *sql.Selector) {
			s.Where(sql.InValues(service.MethodsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.service_methods
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "service_methods" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "service_methods" returned %v for node %v`, *fk, n.ID)
			}
