...
type PermissionQuery struct {
	...
	// eager-loading edges.
	withOwner   *UserQuery
	withClient  *ServiceQuery
	withMethods *MethodQuery
	withFKs     bool
...
func (pq *PermissionQuery) QueryOwner() *UserQuery {
...
func (pq *PermissionQuery) QueryClient() *ServiceQuery {
...
func (pq *PermissionQuery) QueryMethods() *MethodQuery {
	query := &MethodQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withClient:  pq.withClient.Clone(),
		withMethods: pq.withMethods.Clone(),
...
func (pq *PermissionQuery) WithOwner(opts ...func(*UserQuery)) *PermissionQuery {
	query := &UserQuery{config: pq.config}
...
//  WithClient tells the query-builder to eager-loads the nodes that are connected to
// the "client" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PermissionQuery) WithClient(opts ...func(*ServiceQuery)) *PermissionQuery {
	query := &ServiceQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//  WithMethods tells the query-builder to eager-loads the nodes that are connected to
// the "methods" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PermissionQuery) WithMethods(opts ...func(*MethodQuery)) *PermissionQuery {
	query := &MethodQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withMethods = query
	return pq
}
...
func (pq *PermissionQuery) sqlAll(ctx context.Context) ([]*Permission, error) {
	...
	if pq.withOwner != nil || pq.withClient != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withClient; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "permission_client" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Client = n
			}
		}
	}

	if query := pq.withMethods; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Permission)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Methods = []*Method{}
		}
		query.withFKs = true
		query.Where(predicate.Method(func(s *sql.Selector) {
			s.Where(sql.InValues(permission.MethodsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.permission_methods
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "permission_methods" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "permission_methods" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Methods = append(node.Edges.Methods, n)
		}
	}
